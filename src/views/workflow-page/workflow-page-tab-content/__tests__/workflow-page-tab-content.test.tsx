...
import type { WorkflowPageTabContentProps, WorkflowPageTabsContentsMap } from '../workflow-page-tab-content.types';

const MockedTabContent = ({ params }: WorkflowPageTabContentProps) => <div>{JSON.stringify(params)}</div>;
...
describe('WorkflowPageTabContent', () => {
  it('renders tab content with correct params when workflowTab exists in contentsMap', () => {
    const { getByText } = render(<WorkflowPageTabContent params={params} tabsContentMap={mockedTabContentsMap} />);
  ...
  it('does not return any tab cotent if workflowTab is not present in the contentsMap', () => {
    ...
    const { container } = render(<WorkflowPageTabContent params={paramsWithoutTabContent} />);
