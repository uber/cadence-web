const intlStringCompare = new Intl.Collator('en', { sensitivity: 'base' }).compare;

function getSortCompareFunction<ArrayItemType>(sortBy: (arrItem: ArrayItemType) => SortByReturnValue, sortOrder: SortOrder) {
  ...
  return (a: ArrayItemType, b: ArrayItemType): number => {
    ...
    if (typeof aVal === 'string' && typeof bVal === 'string') {
      ...
      else if (aVal === undefined || (aVal === null && bVal !== undefined)) result = -1;
...
function toggleSortOrder({
  ...
  return currentSortColumn === newSortColumn && currentSortOrder === 'ASC' ? 'DESC' : 'ASC';
}
...
function sortBy<ArrayItemType>(
  ...
  return [...results].sort(getSortCompareFunction<ArrayItemType>(sortBy, sortOrder));
}
