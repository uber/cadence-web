describe('Intro', () => {
  it('should provide links to learn about cadence', async function () {
    ...
    linksEl.textNodes('a').should.deep.equal([
  ...
  it('should render the details of a valid domain', async function () {
    const [testEl, scenario] = new Scenario(this.test).go();
    const domainInput = await testEl.waitUntilExists('.intro .domain-navigation input');

    scenario.withDomainDescription('ci-tests');
    domainInput.input('ci-tests');

    const descriptionEl = await testEl.waitUntilExists('.intro .domain-description');
    descriptionEl.should.have.descendant('span.domain-name').with.text('ci-tests');
    descriptionEl.textNodes('dl.details dt').should.deep.equal([
      'description', 'owner', 'Global?', 'Retention Period', 'Emit Metrics', 'Failover Version', 'clusters',
    ]);
    descriptionEl.textNodes('dl.details dd').should.deep.equal([
  ...
  it('should show recent domains with links to them', async function () {
    ...
    const [testEl, scenario] = new Scenario(this.test).go();
    const recentDomains = await testEl.waitUntilExists('.domain-navigation ul.recent-domains');
  ...
  it('should show a description of recent domains when hovered', async function () {
    ...
    const descriptionEl = await testEl.waitUntilExists('.intro .domain-description');
    descriptionEl.textNodes('dl.details dd').should.deep.equal([
      'demo playground', 'ci-test@uber.com', 'No', '3 days', 'Yes', '0', 'ci-test-cluster (active)',
    ]);
  });
});
