...
describe('Execution', () => {
  ...
  async function summaryTest(mochaTest, o) {
    const [scenario, opts] = executionTest(mochaTest, { view: 'summary', ...o });
    scenario.withFullHistory(opts.events);
    const summaryEl = await scenario.render(opts.attach).waitUntilExists('section.execution section.execution-summary dl');
  ...
  describe('Summary', () => {
    ...
    it('should show a full screen view option for input that overflows the area', async function () {
      ...
      const modal = await scenario.vm.$el.waitUntilExists('[data-modal="data-viewer-fullscreen"]');
      await retry(() => {
    ...
    it('should link to the new workflow if the status is ContinuedAsNew', async function () {
      ...
      const wfStatusEl = await summaryEl.waitUntilExists('.workflow-status[data-status="continued-as-new"]');
    ...
    it('should show the failure result from a failed workflow', async function () {
      const [summaryEl] = await summaryTest(this.test, { events: fixtures.history.exampleTimeout });
      const resultsEl = await summaryEl.waitUntilExists('.workflow-result pre');
    ...
    describe('Actions', () => {
      it('should offer the user to terminate a running workflow, prompting the user for a termination reason', async function () {
        const [summaryEl] = await summaryTest(this.test);
        const terminateEl = await summaryEl.waitUntilExists('aside.actions a.terminate');

        terminateEl.trigger('click');
        const confirmTerminateEl = await summaryEl.waitUntilExists('[data-modal="confirm-termination"]');
      ...
      it('should terminate the workflow without a reason', async function () {
        ...
        const terminateEl = await summaryEl.waitUntilExists('[data-modal="confirm-termination"] a.terminate');
      ...
      it('should allow the user to cancel the termination prompt, doing nothing', async function () {
        ...
        const cancelDialog = await summaryEl.waitUntilExists('[data-modal="confirm-termination"] a.cancel');
  ...
  describe('History', () => {
    async function historyTest(mochaTest, o) {
      const [scenario, opts] = executionTest(mochaTest, { view: 'history', ...o });

      scenario.withFullHistory(opts.events);

      const historyEl = await scenario.render(opts.attach).waitUntilExists('section.history');
    ...
    it('should allow the user to change the view format', async function () {
      ...
      const jsonView = await resultsEl.waitUntilExists('pre.json.language-json');
    ...
    it('should allow downloading the full history via export', async function () {
      const [historyEl, scenario] = await historyTest(this.test);
      const exportEl = await scenario.vm.$el.waitUntilExists('section.history .controls a.export');
    ...
    describe('Compact View', function () {
      ...
      it('should focus the timeline when an event is clicked, updating the URL and zooming in', async function () {
        const [timelineEl, compactViewEl, scenario] = await compactViewTest(this.test);
        ...
        const failedActivity = await compactViewEl.waitUntilExists('.timeline-event.activity.failed');
      ...
      xit('should scroll the event into view if an event is clicked from the timeline, updating the URL', async function () {
        ...
        scenario.location.should.equal('/domain/ci-test/workflows/email-daily-summaries/emailRun1/history?format=compact');
        const failedActivity = await timelineEl.waitUntilExists('.vis-range.activity.failed');
      ...
      it('should show event details when an event is clicked', async function () {
        ...
        const childWf = await compactViewEl.waitUntilExists('.timeline-event.child-workflow.completed ');
    ...
    describe('Grid View', () => {
      ...
      it('should allow toggling of the time column between elapsed and local timestamp', async function () {
        ...
        const [elapsedEl, tsEl] = historyEl.querySelectorAll('.thead .th:nth-child(3) a');
      ...
      it('should show details as flattened key-value pairs from parsed json, except for result and input', async function () {
        const [historyEl] = await historyTest(this.test);
        const startDetails = await historyEl.waitUntilExists('.results .tr:first-child .td:nth-child(4)');
        const inputPreText = JSON.stringify(fixtures.history.emailRun1[0].details.input, null, 2);
      ...
      it('should show a full screen view option for large JSON fields, and allow copying it', async function () {
        ...
        const viewFullScreen = await historyEl.waitUntilExists('.results .td:nth-child(4) .data-viewer.overflow a.view-full-screen');
      ...
      it('should allow toggling of the details column between summary and full details', async function () {
        ...
        const [summaryEl, fullDetailsEl] = historyEl.querySelectorAll('.thead .th:nth-child(4) a');
      ...
      it('should link to child workflows, and load its history when navigated too', async function () {
        ...
        const childStartDetails = await historyEl.waitUntilExists('.results .vue-recycle-scroller__item-view:nth-child(1) .tr .td:nth-child(4)');
  ...
  describe('Stack Trace', () => {
    ...
    it('should show the current stack trace', async function () {
      const [scenario, opts] = executionTest(this.test, { view: 'stack-trace' });

      scenario.withFullHistory().api.postOnce(`${scenario.execApiBase()}/queries/__stack_trace`, {
        queryResult: 'goroutine 1:\n\tat foo.go:56',
      });

      const stackTraceEl = await scenario.render().waitUntilExists('section.stack-trace');
  ...
  describe('Queries', () => {
    async function queriesTest(mochaTest, queries) {
      ...
      const queriesEl = await scenario.render().waitUntilExists('section.execution section.queries');
    ...
    it('should query the list of stack traces, and show it in the dropdown, enabling run as appropriate', async function () {
      const [queriesEl] = await queriesTest(this.test, ['__stack_trace', 'foo', 'bar']);

      const queriesDropdown = await queriesEl.waitUntilExists('.query-name .dropdown');
    ...
    it('should show an error if queries could not be listed', async function () {
      const [queriesEl] = await queriesTest(this.test, { status: 400, body: { message: 'I do not understand' } });
