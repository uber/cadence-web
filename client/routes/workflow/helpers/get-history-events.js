...
const getHistoryEvents = ({
  ...
  return events
    ...
    .map((event, index, eventList) => {
      const timeStampDisplay = getTimeStampDisplay({ dateFormat, event, index, timeFormat, timezone });
      const timeElapsedDisplay = getTimeElapsedDisplay({ dateFormat, event, eventList, index, timeFormat, timezone });
